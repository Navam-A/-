...
export default function EditBook() {
  ...
  return (
    <div className="w-full mx-auto grid grid-cols-2 lg:grid-cols-3 my-5 p-3">
      {bookData.map((bk) => {
        return (
          <div className="relative group w-[330px]" key={bk.id}>
            <div className="group-hover:opacity-50">
              <BookTemplate price={bk.price} title={bk.title} path={bk.image} author={bk.author}/>
            </div>
            <span className="absolute inset-0 hidden group-hover:flex group-hover:items-center group-hover:justify-center">
              ...
              <button
                onClick={(e) => {
                  e.preventDefault();
                  axios
                    .delete(`http://localhost:3000/books/${bk.id}`)
                    .then(() => alert("Books Deleted Successfully !"))
                    .catch((err) => console.log(err));
                }}
